...
export function useMySessions(filters = {}) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await api.get("/api/sessions/mine", {
        params: buildSessionParams(filters),
      });
      return response.data;
    },
...
export function useAddDrillToSession() {
  ...
  return useMutation({
    ...
    onError: (error) => {
      const message =
        error.response?.data?.detail || "Failed to add drill to session";
      toast.error(message);
    },
  });
}
...
export function useReorderSessionDrills() {
  ...
  return useMutation({
    mutationFn: async ({ sessionId, drill_session_ids }) => {
      const response = await api.patch(`/api/sessions/${sessionId}/drills`, {
        drill_session_ids,
      });
      return response.data;
    },
    ...
    onError: (error) => {
      const message =
        error.response?.data?.detail || "Failed to reorder drills";
      toast.error(message);
    },
  });
}
  ...
  return useMutation({
    mutationFn: async ({ sessionId, drillSessionId, data }) => {
      const response = await api.patch(
        `/api/sessions/${sessionId}/drills/${drillSessionId}`,
        data,
      );
      return response.data;
    },
    ...
    onError: (error) => {
      const message =
        error.response?.data?.detail || "Failed to update drill in session";
      toast.error(message);
    },
  });
}
...
export function useRemoveDrillFromSession() {
  ...
  return useMutation({
    ...
    onError: (error) => {
      const message =
        error.response?.data?.detail || "Failed to remove drill from session";
      toast.error(message);
    },
  });
}
