...
const COLOR_SWATCHES = [
  '#cc1414', '#ffffff', '#FFD700', '#4488FF',
  '#44BB44', '#FF8800', '#ff69b4', '#888888',
]
...
function CourtBackground() {
  ...
  const zoneLabels = [
    { label: '4', x: COURT_X + (attackL - COURT_X) / 2,               y: COURT_Y + COURT_H / 4 },
    { label: '3', x: (attackL + CENTER_X) / 2,                         y: COURT_Y + COURT_H / 4 },
    { label: '2', x: CENTER_X + (attackR - CENTER_X) / 2,             y: COURT_Y + COURT_H / 4 },
    { label: '1', x: attackR + (COURT_X + COURT_W - attackR) / 2,     y: COURT_Y + COURT_H / 4 },
    { label: '5', x: COURT_X + (attackL - COURT_X) / 2,               y: COURT_Y + (COURT_H * 3) / 4 },
    { label: '6', x: (attackL + CENTER_X) / 2,                         y: COURT_Y + (COURT_H * 3) / 4 },
  ]

  return (
    <>
      <Rect x={COURT_X} y={COURT_Y} width={COURT_W} height={COURT_H} fill={COURT_BG} />
      <Rect x={COURT_X} y={COURT_Y} width={COURT_W} height={COURT_H}
        stroke={COURT_LINE} strokeWidth={2} fill="transparent" />
      {/* Net */}
      <Line points={[CENTER_X, COURT_Y, CENTER_X, COURT_Y + COURT_H]}
        stroke={NET_COLOR} strokeWidth={2.5} />
      {/* Attack lines */}
      <Line points={[attackL, COURT_Y, attackL, COURT_Y + COURT_H]}
        stroke={ATTACK_COLOR} strokeWidth={1.5} dash={[6, 4]} />
      <Line points={[attackR, COURT_Y, attackR, COURT_Y + COURT_H]}
        stroke={ATTACK_COLOR} strokeWidth={1.5} dash={[6, 4]} />
      <Text x={CENTER_X - 14} y={COURT_Y - 20}
        text="NET" fontSize={10} letterSpacing={2}
        fill={NET_COLOR} listening={false} />
    </>
  )
}
...
export default function FieldCanvas({ stageRef, initialDrawing }) {
  const {
    icons, arrows, selectedId,
    setSelected, addIcon, addArrow,
    updateIcon, updateArrow, removeIcon, removeArrow,
    undo, redo, loadDrawing,
  } = useDrawingStore()
  ...
  function handleStageMouseUp() {
    ...
    if (Math.hypot(x2 - x1, y2 - y1) > 15) {
      addArrow({ id: crypto.randomUUID(), points: [x1, y1, x2, y2], color: '#ffffff', dashed: false })
    }
  ...
  function handleIconContextMenu(e, iconId) {
    ...
    if (e?.isTextEdit) {
      ...
      if (!icon) return
      setTextEdit({ iconId, stageX: icon.x, stageY: icon.y, currentText: icon.text || 'Label' })
      return
    }
  ...
  return (
    <div style={{ position: 'relative', display: 'inline-block' }}>
      {/* Text editor overlay */}
      {textEdit && (
        <div style={{ position: 'absolute', left: textEdit.stageX, top: textEdit.stageY, zIndex: 200 }}>
          <input
            autoFocus
      ...
      >
        <Stage
          ...
          width={STAGE_W}
          height={STAGE_H}
          style={{ backgroundColor: '#111111', borderRadius: 8, display: 'block' }}
          ...
          <Layer name="arrows">
            {arrows.map((arrow) => (
              <Arrow
                ...
                onClick={(e) => { e.cancelBubble = true; setSelected(arrow.id) }}
      ...
      {menu && (
        ...
        >
          ...
          {/* Text icon: edit text */}
          {menu.targetType === 'icon' && icons.find((i) => i.id === menu.targetId)?.type === 'text' && (
            <button
              style={menuBtnStyle}
              onClick={() => {
                const icon = icons.find((i) => i.id === menu.targetId)
                setTextEdit({ iconId: icon.id, stageX: icon.x, stageY: icon.y, currentText: icon.text || 'Label' })
                setMenu(null)
              }}
            >
              Edit text
          ...
          <button style={{ ...menuBtnStyle, color: '#ff3333' }} onClick={deleteMenuTarget}>
            Delete
          </button>
        </div>
      )}
      ...
      {/* Usage hint */}
      <div style={{ position: 'absolute', bottom: 6, right: 10, fontSize: 10, color: 'rgba(255,255,255,0.22)', pointerEvents: 'none' }}>
        Drag icons from palette · Click &amp; drag court to draw arrows · Right-click to edit/delete
      </div>
    </div>
  )
}
