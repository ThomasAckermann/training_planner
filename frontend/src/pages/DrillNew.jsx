...
import { useCreateDrill, useSaveDrawing } from '../hooks/useDrills.js'
...
export default function DrillNew() {
  ...
  const { getDrawingJson, icons, arrows, reset: resetDrawing } = useDrawingStore()
  ...
  async function onSubmit(values) {
    ...
    const drill = await createDrill.mutateAsync(payload)

    // Save drawing if canvas has content
    if (icons.length > 0 || arrows.length > 0) {
      const drawingJson = getDrawingJson()
      const thumbnailDataUrl = stageRef.current?.toDataURL({ mimeType: 'image/png', pixelRatio: 2 })
      await saveDrawing.mutateAsync({ drillId: drill.id, drawingJson, thumbnailDataUrl })
    }
    ...
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
      <div className="mb-6">
        ...
        <p style={{ color: 'var(--color-text-muted)' }}>
          Fill in the details, then optionally draw a tactical diagram.
        </p>
      </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <div className="flex gap-3">
              ...
              <Button type="submit" variant="primary" loading={isSubmitting || createDrill.isPending || saveDrawing.isPending}>
                Create Drill
              </Button>
            </div>
          </div>
      ...
      {activeTab === 'drawing' && (
        <div className="space-y-4">
          <p className="text-sm" style={{ color: 'var(--color-text-muted)' }}>
            Draw a tactical diagram for this drill. The drawing will be saved automatically when you create the drill.
          </p>
          <ErrorBoundary
          ...
          >
            <Suspense fallback={<div className="h-64 rounded-xl animate-pulse" style={{ backgroundColor: 'var(--color-surface)' }} />}>
              <DrawingToolbar
                stageRef={stageRef}
                onSave={() => toast('Drawing will be saved automatically when you create the drill', { icon: '💾' })}
                isSaving={false}
