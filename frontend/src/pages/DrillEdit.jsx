...
import { useDrill, useUpdateDrill, useSaveDrawing } from '../hooks/useDrills.js'
import { AGE_RANGES, EQUIPMENT_OPTIONS, FOCUS_AREAS, SKILL_LEVELS } from '../lib/constants.js'
...
const DrawingToolbar = lazy(() => import('../components/drawing/DrawingToolbar.jsx'))

const schema = z.object({
  title: z.string().min(1, 'Title is required').max(200),
  description: z.string().optional(),
  duration_minutes: z.coerce.number().int().positive().optional().or(z.literal('')),
  num_players_min: z.coerce.number().int().positive().optional().or(z.literal('')),
  num_players_max: z.coerce.number().int().positive().optional().or(z.literal('')),
...
export default function DrillEdit() {
  ...
  const { reset, handleSubmit, register, formState: { errors, isSubmitting } } =
  ...
  function addEquipment(item) {
    if (item && !equipment.includes(item)) setEquipment((prev) => [...prev, item])
  }
  function removeEquipment(item) { setEquipment((prev) => prev.filter((e) => e !== item)) }
  function addTag(e) {
    if (e.key === 'Enter' || e.key === ',') {
      e.preventDefault()
      const tag = tagInput.trim().toLowerCase().replace(/\s+/g, '-')
      if (tag && !skillTags.includes(tag)) setSkillTags((prev) => [...prev, tag])
      setTagInput('')
    }
  }
  function removeTag(tag) { setSkillTags((prev) => prev.filter((t) => t !== tag)) }
  ...
  async function onSubmit(values) {
    const payload = {
      ...
      num_players_min: values.num_players_min ? Number(values.num_players_min) : null,
      num_players_max: values.num_players_max ? Number(values.num_players_max) : null,
  ...
  async function handleSaveDrawing({ drawingJson, thumbnailDataUrl }) {
    await saveDrawing.mutateAsync({ drillId: id, drawingJson, thumbnailDataUrl })
  }
  ...
  if (!drill) {
    return (
      <div className="max-w-5xl mx-auto px-4 py-20 text-center">
        <p style={{ color: 'var(--color-danger)' }} className="mb-4">Drill not found</p>
        <Button variant="secondary" onClick={() => navigate('/me')}>Back to My Drills</Button>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-10">
      ...
      {/* Tab bar */}
      <div className="flex gap-1 mb-6 p-1 rounded-xl border"
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          <Card>
            ...
            <div className="space-y-4">
              <Input label="Title *" placeholder="e.g. 6-2 Serve Receive Pattern"
                error={errors.title?.message} {...register('title')} />
              <div>
                <label className={labelClass} style={{ color: 'var(--color-text-primary)' }}>Description</label>
                <textarea className={textareaClass} rows={5}
                  placeholder="Describe the drill, setup, and coaching points..."
                  {...register('description')} />
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <Input label="Duration (min)" type="number" placeholder="15"
                  error={errors.duration_minutes?.message} {...register('duration_minutes')} />
                <Input label="Min Players" type="number" placeholder="6"
                  error={errors.num_players_min?.message} {...register('num_players_min')} />
                <Input label="Max Players" type="number" placeholder="12"
                  error={errors.num_players_max?.message} {...register('num_players_max')} />
              </div>
            </div>
          </Card>
          ...
          <Card>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              <div>
                <label className={labelClass} style={{ color: 'var(--color-text-primary)' }}>Age Range</label>
              ...
              <div>
                <label className={labelClass} style={{ color: 'var(--color-text-primary)' }}>Skill Level</label>
              ...
              <div>
                <label className={labelClass} style={{ color: 'var(--color-text-primary)' }}>Focus Area</label>
          ...
          <Card>
            <h2 className="text-lg font-semibold mb-4" style={{ color: 'var(--color-text-primary)' }}>Equipment</h2>
            <div className="flex flex-wrap gap-2 mb-3">
              {EQUIPMENT_OPTIONS.map((item) => {
                const selected = equipment.includes(item)
                return (
                  <button key={item} type="button"
                    onClick={() => (selected ? removeEquipment(item) : addEquipment(item))}
                    className="px-3 py-1 rounded-full text-sm border transition-all"
                    style={{
                      backgroundColor: selected ? '#cc141422' : 'var(--color-surface2)',
          ...
          <Card>
            <h2 className="text-lg font-semibold mb-4" style={{ color: 'var(--color-text-primary)' }}>Skill Tags</h2>
            <div className="flex flex-wrap gap-1.5 mb-3">
              {skillTags.map((tag) => (
                <span key={tag} className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-mono"
                  style={{ backgroundColor: '#cc141422', color: '#cc1414', border: '1px solid #cc141444' }}>
                  {tag}
                  <button type="button" onClick={() => removeTag(tag)}><X className="w-3 h-3" /></button>
                </span>
              ))}
            </div>
            <input type="text" className={textareaClass.replace('resize-none', '')}
              placeholder="Type a tag and press Enter (e.g. jump-serve)"
              value={tagInput} onChange={(e) => setTagInput(e.target.value)}
              onKeyDown={addTag} style={{ height: 'auto' }} />
          ...
          <Card>
            ...
            <Input label="YouTube or Vimeo URL" placeholder="https://www.youtube.com/watch?v=..."
              error={errors.video_url?.message} {...register('video_url')} />
          </Card>
          ...
          <div className="flex items-center justify-between pt-2">
            ...
            <div className="flex gap-3">
              <Button type="button" variant="ghost" onClick={() => navigate(`/drills/${id}`)}>Cancel</Button>
              <Button type="submit" variant="primary" loading={isSubmitting || updateDrill.isPending}>
                Save Changes
              </Button>
            </div>
          </div>
      ...
      {activeTab === 'drawing' && (
        <div className="space-y-4">
          <p className="text-sm" style={{ color: 'var(--color-text-muted)' }}>
            Draw a tactical diagram. Right-click icons or arrows to change color / delete.
          ...
          >
            <Suspense fallback={<div className="h-64 rounded-xl animate-pulse" style={{ backgroundColor: 'var(--color-surface)' }} />}>
              <DrawingToolbar
                stageRef={stageRef}
              ...
              <div className="flex gap-4 mt-3">
                <IconPalette />
                <FieldCanvas stageRef={stageRef} initialDrawing={drill.drawing_json} />
              </div>
            </Suspense>
          </ErrorBoundary>
        </div>
