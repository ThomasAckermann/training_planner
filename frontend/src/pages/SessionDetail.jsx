...
import { ArrowLeft, Pencil, Trash2, Clock, Layers, Users, Heart, Copy, FileDown } from 'lucide-react'
import { useSession, useDeleteSession, useLikeSession, useDuplicateSession } from '../hooks/useSessions.js'
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-10">
        <div className="animate-pulse space-y-4">
          ...
          <div className="h-12 w-2/3 rounded" style={{ backgroundColor: 'var(--color-surface)' }} />
  ...
  if (isError || !session) {
    return (
      <div className="max-w-4xl mx-auto px-4 py-10 text-center">
        <p style={{ color: 'var(--color-danger)' }}>Session not found or access denied.</p>
        <Button variant="secondary" className="mt-4" onClick={() => navigate('/explore')}>
          Browse Sessions
        </Button>
      </div>
    )
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="mb-6">
        ...
        {/* Stats */}
        <div className="flex flex-wrap items-center gap-6 text-sm mb-6" style={{ color: 'var(--color-text-muted)' }}>
        ...
        {session.description && (
          ...
          >
            <p className="text-sm leading-relaxed" style={{ color: 'var(--color-text)', fontFamily: '"Lora", serif' }}>
              {session.description}
            </p>
          </div>
        )}
      ...
      <div className="flex items-center justify-center gap-3 pt-4 border-t flex-wrap" style={{ borderColor: 'var(--color-border)' }}>
        <Button
          variant="secondary"
          onClick={handleLike}
        ...
        >
          ...
          {session.likes_count > 0 ? `${session.likes_count} Likes` : 'Like this session'}
        </Button>
        {currentUser && !isOwner && (
          <Button
            variant="secondary"
