...
import { Camera, PlusCircle, Pencil, Trash2, Eye, EyeOff, Layers, Clock } from 'lucide-react'
...
export default function Dashboard() {
  ...
  const { data: drillData, isLoading: drillsLoading } = useMyDrills(drillFilters)
  const { data: sessionData, isLoading: sessionsLoading } = useMySessions(sessionFilters)
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
      ...
      <div className="mb-8 flex items-start gap-5">
        ...
        <div>
          ...
          <p className="mt-1 text-sm" style={{ color: 'var(--color-text-muted)' }}>
            Welcome back, {user?.name} &middot; {drillTotal} drill{drillTotal !== 1 ? 's' : ''} &middot; {sessionTotal} session{sessionTotal !== 1 ? 's' : ''}
          </p>
        </div>
      </div>

      {/* Tab bar */}
      <div className="flex gap-1 mb-8 p-1 rounded-xl border" style={{ backgroundColor: 'var(--color-surface)', borderColor: 'var(--color-border)', width: 'fit-content' }}>
        {[{ key: 'sessions', label: 'My Sessions' }, { key: 'drills', label: 'My Drills' }].map(({ key, label }) => (
          <button
            key={key}
      ...
      {/* ── My Sessions ── */}
      {tab === 'sessions' && <section className="mb-12">
              ...
              <Card key={session.id} hoverable className="cursor-pointer">
                <div className="flex items-center gap-4">
                  ...
                  <div
                    ...
                    style={{
                      backgroundColor: session.is_public ? '#cc141422' : '#88888822',
                    }}
                    ...
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      ...
                      {!session.is_public && (
                        <span
                          ...
                          style={{ backgroundColor: '#88888822', color: 'var(--color-text-muted)' }}
                        >
                          draft
                        </span>
                      )}
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0">
                    <Button
                      variant="secondary"
                      size="sm"
                      onClick={(e) => { e.stopPropagation(); navigate(`/sessions/${session.id}/edit`) }}
                    >
      ...
      {tab === 'drills' && <section>
        <div className="flex items-center justify-between mb-4">
          ...
          <Button variant="primary" size="sm" onClick={() => navigate('/drills/new')}>
            <PlusCircle className="w-4 h-4" />
            New Drill
          </Button>
        </div>
        ...
        >
          <DrillFilters filters={drillFilters} onFiltersChange={setDrillFilters} />
        </div>
                ...
                <Card key={drill.id} hoverable className="cursor-pointer">
                  <div className="flex items-center gap-4">
                    <div
                      className="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center"
                      style={{
                        backgroundColor: drill.is_public ? '#cc141422' : '#88888822',
                      }}
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <Button
                        variant="secondary"
                        size="sm"
                        onClick={(e) => { e.stopPropagation(); navigate(`/drills/${drill.id}/edit`) }}
                      >
                        <Pencil className="w-3.5 h-3.5" />
                        Edit
                      </Button>
